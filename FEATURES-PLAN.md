...
- [ ] A1. Lock epic owner to Lead in board detail modal (no assignee select for epics; show "Owned by Team Lead").
- [ ] A2. Board epic filter → dropdown `select` (keep `data-testid="epic-filter"`); remove chip component; persist last selection per project (localStorage).

### B. Architect agent

- [ ] B1. Add `architect` to catalog (design-focused prompt + skills, emoji/color).
- [ ] B2. `planEpic`: insert Architect design step after PM → architect writes the epic's living plan (design: approach, components, risks, streams) and posts a design summary to chat. Lead then assigns the decomposed tasks (task creation stays deterministic per available specialist so tests stay green; attributed to the architect's design). Fake-adapter design marker as needed.

### C. Team Lead proactive manager loop

- [ ] C1. `LeadManager` in orchestrator: event-driven (debounced on board/agent events) + periodic heartbeat via SchedulerService.
- [ ] C2. Deterministic assignment: assign unassigned `todo` items with satisfied deps to best-matching agent (stream/skills); requeue via `onItemAssigned`. This is how review fix-tasks and user-added cards get picked up — the **Lead** assigns, agents never self-pick.
- [ ] C3. Monitoring/guidance: detect blocked/stalled/needs_input agents; Lead posts concise guidance (throttled; deterministic/no-op in fake mode unless triggered). Call a group discussion on cross-stream conflict or repeated review failures.
- [ ] C4. Ownership heartbeat: Lead posts a throttled status/next-steps note to main chat while an epic is active.

### D. PR review with comments (Architect reviews)

- [ ] D1. `PrComment` entity + `pr_comments` table + store CRUD + `pr_comment.updated` WS event; bundle carries comments.
- [ ] D2. Architect is the reviewer (fallback reviewer→qa→security). Reviewer app tool `add_review_comment(body, targetStream)`; fake marker `[[REVIEW_COMMENT: stream | body]]`.
- [ ] D3. Each comment → a fix task the **Lead assigns** to the target agent; comment `status: open`. Comment resolves when its fix task reaches review/done.
- [ ] D4. UI: comments listed on the PR page with status + assignee; resolve indicator.

### E. Lead approves + merges

- [ ] E1. Approval gated on **all comments resolved** + reviewer re-check; only the **Team Lead** approves+merges.
- [ ] E2. On approve: close children + epic, PR → merged, progress 100, notify. Keep `MAX_REVIEW_ITER` deadlock guard → escalate to user if exceeded.
